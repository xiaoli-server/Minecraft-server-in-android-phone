...
# 安全写入mcserver13.sh的第12行
if result != "":
    try:
        # 读取mcserver13.sh的所有行
        with open("mcserver13.sh", "r") as f:
            lines = f.readlines()
        ...
        # 写回文件
        with open("mcserver13.sh", "w") as f:
            f.writelines(lines)
        print("已成功将结果写入mcserver13.sh的第12行！")
    except FileNotFoundError:
        print("错误：找不到mcserver13.sh文件，请确保脚本和mcserver12.sh在同一个文件夹！")

# 替换mcserver12.sh的13,15,22行版本号
if num:
    try:
        with open("mcserver13.sh", "r") as f:
            lines = f.readlines()
       ...
       # 替换第13行（索引12）的1.21.11
        ...
        if len(lines) >= 15:
            ...
            # 替换第16行（索引15）的1.21.11
        if len(lines) >= 16:
            lines[15] = lines[15].replace("1.21.11", num)
            
            # 替换第19行（索引18）的1.21.11
        if len(lines) >= 19:
            lines[18] = lines[18].replace("1.21.11", num)
            
            # 替换第20行（索引19）的1.21.11
        if len(lines) >= 20:
            lines[19] = lines[19].replace("1.21.11", num)
            
            # 替换第21行（索引20）的1.21.11
        if len(lines) >= 21:
            lines[20] = lines[20].replace("1.21.11", num)
        
        # 替换第22行（索引21）的1.21.11
        if len(lines) >= 22:
            lines[21] = lines[21].replace("1.21.11", num)
        
        with open("mcserver13.sh", "w") as f:
            f.writelines(lines)
        print(f"已替换13、15、22行版本号为：{num}")
    except FileNotFoundError:
        print("找不到mcserver13.sh文件！")
